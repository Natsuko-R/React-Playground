...
export default function IrrigationRatebtn(props) {
  const clickIrrigationRate = props.clickIrrigationRate;
  const [clicked, setClicked] = useState(false);
  const handleclick = ()=>{
    setClicked(!clicked);
    props.setclickIrrigationRate( clickIrrigationRate => !clickIrrigationRate );
  }
  ...
  return (
    <div>
        <button className={buttonClass} onClick={handleclick}>
          <div className='col-start-1 col-span-7 p-2 text-xs text-left'>A02R002:潅水バルブ</div>
          <div className='col-start-3 col-span-4 text-xl text-center font-bold'>
            {52.19}%
          </div>
          <span className='col-start-1 col-span-2 w-12 p-1 ml-2 mb-2 text-xs text-center font-bold text-white rounded-md bg-orange-300'>流量</span>
