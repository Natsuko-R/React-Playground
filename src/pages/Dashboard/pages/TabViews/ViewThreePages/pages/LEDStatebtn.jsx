...
export default function LEDStatebtn(props) {
  const clickLEDState = props.clickLEDState;
  const [clicked, setClicked] = useState(false);
  const handleclick = ()=>{
    setClicked(!clicked);
    props.setclickLEDState( clickLEDState => !clickLEDState );
  }
  ...
  return (
    <div>
        <button className={buttonClass} onClick={handleclick}>
          <div className='col-start-1 col-span-7 p-2 text-xs text-left'>A01R005:LED1</div>
          <div className='col-start-3 col-span-4 text-xl text-center font-bold'>
            <FontAwesomeIcon icon={faLightbulb} className='text-yellow-300' />作動
          </div>
          <span className='col-start-1 col-span-2 w-12 p-1 ml-2 mb-2 text-xs text-center font-bold text-white rounded-md bg-lime-400'>リレー</span>
