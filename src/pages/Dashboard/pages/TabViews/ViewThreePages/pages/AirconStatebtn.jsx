...
export default function AirconStatebtn(props) {
  const clickAirconState = props.clickAirconState;
  const [clicked, setClicked] = useState(false);
  const handleclick = ()=>{
    setClicked(!clicked);
    props.setclickAirconState( clickAirconState => !clickAirconState );
  }
...
return (
  <div>
      <button className={buttonClass} onClick={handleclick}>
        <div className='col-start-1 col-span-7 p-2 text-xs text-left'>H01R003:冷房</div>
          <div className='col-start-3 col-span-4 text-xl text-center font-bold'>
            停止
          </div>
          <span className='col-start-1 col-span-2 w-12 p-1 ml-2 mb-2 text-xs text-center font-bold text-white rounded-md bg-blue-800'>リレー</span>
