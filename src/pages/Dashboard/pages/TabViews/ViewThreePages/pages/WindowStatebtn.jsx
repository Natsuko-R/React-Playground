...
import { faThermometerHalf, faTint, faSolarPanel, faSmog, faWindowClose } from '@fortawesome/free-solid-svg-icons';

export default function WindowStatebtn(props) {
  const clickWindowState = props.clickWindowState;
  const [clicked, setClicked] = useState(false);
  const handleclick = ()=>{
    setClicked(!clicked);
    props.setclickWindowState( clickWindowState => !clickWindowState );
  }
  ...
  return (
    <div>
        <button className={buttonClass} onClick={handleclick}>
          <div className='col-start-1 col-span-7 p-2 text-xs text-left'>H01R025:汎用２</div>
          <div className='col-start-3 col-span-4 text-xl text-center font-bold'>
          <FontAwesomeIcon icon={faWindowClose} className='text-yellow-500' />停止
          </div>
          <span className='col-start-1 col-span-2 w-12 p-1 ml-2 mb-2 text-xs text-center font-bold text-white rounded-md bg-purple-300'>リレー</span>
