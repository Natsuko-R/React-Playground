...
export default function ECLiquidFlowRatebtn(props) {
  const clickECLiquidFlowRate = props.clickECLiquidFlowRate;
  const [clicked, setClicked] = useState(false);
  const handleclick = ()=>{
    setClicked(!clicked);
    props.setclickECLiquidFlowRate( clickECLiquidFlowRate => !clickECLiquidFlowRate );
  }
  ...
  return (
    <div>
        <button className={buttonClass} onClick={handleclick}>
          <div className='col-start-1 col-span-7 p-2 text-xs text-left'>A02R002:ECバルブ</div>
          <div className='col-start-3 col-span-4 text-xl text-center font-bold'>
            {52.19}%
          </div>
          <span className='col-start-1 col-span-2 w-12 p-1 ml-2 mb-2 text-xs text-center font-bold text-white rounded-md bg-pink-400'>流量</span>
