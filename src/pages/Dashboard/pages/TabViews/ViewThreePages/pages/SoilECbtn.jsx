...
export default function SoilECbtn(props) {
  const clickSoilEC = props.clickSoilEC;
  const [clicked, setClicked] = useState(false);
  const handleclick = ()=>{
    setClicked(!clicked);
    props.setclickSoilEC( clickSoilEC => !clickSoilEC );
  }
  ...
  return (
    <div>
        <button className={buttonClass} onClick={handleclick}>
          <div className='col-start-1 col-span-7 p-2 text-xs text-left'>A01S002:土壌測定器</div>
          <div className='col-start-3 col-span-4 text-xl text-center font-bold'>
            {1.15}
          </div>
          <span className='col-start-1 col-span-2 w-12 p-1 ml-2 mb-2 text-xs text-center font-bold text-white rounded-md bg-stone-400'>EC</span>
